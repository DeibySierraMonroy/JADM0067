...
/**
 * Implementa todos los metodos de acceso y manipulacion de datos de la entidad
 * RHU.LIBROINGRESO_DOCUMENTO
 *
...
public class DocumentsRepoImpl implements DocumentsRepo {
    ...
    @Override
    public DatabaseResultDto<LibroIngresoDocumentoDto> getAllDocuments(Long libConsecutivo) {
        try (Connection connection = OracleConnection.getConnection()) {
            ...
            try (CallableStatement callableStatement = connection.prepareCall(query)) {
                ...
                while (resultSet.next()) {
                    ...
                    documentoDto.setLdoEstado(resultSet.getString("ldo_estado"));
    ...
    public DatabaseResultDto<LibroIngresoDocumentoDto> updateDocument(LibroIngresoDocumentoDto documento) {
        try (Connection connection = OracleConnection.getConnection()) {
            String query = "call ADM.QB_APP_JADM0065_LOGIC.PL_ACTUALIZAR_DOCUMENTO(?,?)";
            try (CallableStatement callableStatement = connection.prepareCall(query)) {
